 ...
 *
 *  • Logistic-regression models trained with full-batch & mini-batch SGD
 *  • Algorithms: FedAvg, FedProx, SCAFFOLD (Control Variates),
 *    FL + DP-SGD (Differentially Private SGD), and Centralized Baseline.
...
export function randnLocal(rng: () => number): number {
  let u = 0;
  let v = 0;
  while (u === 0) u = rng();
  while (v === 0) v = rng();
...
export function predictSingle(run: RunResult, rawFeatures: number[]): PredictionResult {
  ...
  for (let j = 0; j < d; j++) {
    const std = ds.std[j] || 1;
    const mean = ds.mean[j] || 0;
    const rawVal = rawFeatures[j] ?? mean;
    const stdVal = (rawVal - mean) / std;
    const w = run.weights[j] ?? 0;
    const c = w * stdVal;
    z += c;
    contributions.push({
      feature: ds.meta.features[j].label,
      impact: c,
    });
  }
  ...
  const label = isPos ? ds.meta.positiveLabel : ds.meta.negativeLabel;
...
function localTrain(
  ...
  cGlobal?: Float64Array | null,
  cLocal?: Float64Array | null
): { w: Float64Array; b: number; newCLocal?: Float64Array } {
  const d = w0.length;
  ...
  for (let e = 0; e < epochs; e++) {
    ...
    for (let j = 0; j < d; j++) {
      ...
      const scaffoldCorrection = cGlobal && cLocal ? cGlobal[j] - cLocal[j] : 0;
      w[j] -= lr * (g[j] + scaffoldCorrection + 0.0004 * w[j] + prox);
    }
    b -= lr * gb;
  }

  let newCLocal: Float64Array | undefined;
  if (cGlobal && cLocal) {
    newCLocal = new Float64Array(d);
    const steps = Math.max(1, epochs);
    for (let j = 0; j < d; j++) {
      newCLocal[j] = cLocal[j] - cGlobal[j] + (w0[j] - w[j]) / (steps * lr);
    }
  }

  return { w, b, newCLocal };
}
...
export async function trainFederated(
  ...
  const isDpsgd = config.algo === "dpsgd";
  const isScaffold = config.algo === "scaffold";
  const isCentral = config.algo === "central";
  const isDpActive = config.dp || isDpsgd;

  log("info", `Dataset "${ds.meta.name}" · ${ds.meta.nSamples} rows · ${d} features`);
  log("priv", `Algorithm: ${config.algo.toUpperCase()} · Raw data never leaves clients.`);
  log(
    "info",
  ...
  // SCAFFOLD control variate vectors
  const cGlobal = isScaffold ? new Float64Array(d) : null;
  const cLocals = isScaffold ? clients.map(() => new Float64Array(d)) : null;
  ...
  for (let r = 1; r <= config.rounds; r++) {
    ...
    log("info", `Round ${r}/${config.rounds} — broadcasting global model wₜ (${config.algo.toUpperCase()}) to ${nPart} sampled clients`, r);
    ...
    const deltaCSum = isScaffold ? new Float64Array(d) : null;

    for (const c of participants) {
      if (cancel.cancelled) throw new Error("cancelled");
      const k = clients.indexOf(c);
      const idx = splits[k];
      const cLocalK = cLocals ? cLocals[k] : null;
      ...
      const local = localTrain(
        ...
        config.localEpochs,
        config.learningRate,
        config.algo === "fedprox" ? config.mu : 0,
        w,
        cGlobal,
        cLocalK
      );
      ...
      if (isScaffold && local.newCLocal && cLocalK && cLocals && deltaCSum) {
        const newC = Float64Array.from(local.newCLocal);
        for (let j = 0; j < d; j++) {
          deltaCSum[j] += newC[j] - cLocalK[j];
        }
        cLocals[k] = newC;
      }
      ...
      // DP step 1 — clip update to L2 ball of radius C
      let clipped = false;
      const effectiveClipNorm = isDpsgd ? Math.min(config.clipNorm, 0.8) : config.clipNorm;
      if (isDpActive && norm > effectiveClipNorm) {
        const s = effectiveClipNorm / norm;
      ...
      log(
        "info",
        `${c.name} (${c.org}) trained ${config.localEpochs} epoch${config.localEpochs > 1 ? "s" : ""} on ${idx.length} local rows · local acc ${(acc * 100).toFixed(1)}%${clipped ? ` · update clipped ‖Δw‖≤${effectiveClipNorm}` : ""}`,
        r
      );
    }
    await sleep(config.speedMs * 0.55);

    // Update global SCAFFOLD control variate
    if (isScaffold && cGlobal && deltaCSum) {
      for (let j = 0; j < d; j++) {
        cGlobal[j] += deltaCSum[j] / participants.length;
      }
      log("priv", `SCAFFOLD: Updated global control variate c vector across ${participants.length} nodes`, r);
    }
    ...
    // DP step 2 — Gaussian mechanism addition
    let sigma = 0;
    if (isDpActive) {
      const effectiveClipNorm = isDpsgd ? Math.min(config.clipNorm, 0.8) : config.clipNorm;
      const sensitivity = (2 * effectiveClipNorm) / updates.length;
      ...
      log("priv", `${isDpsgd ? "FL + DP-SGD" : "DP"} applied: clip C=${effectiveClipNorm}, σ=${sigma.toFixed(3)} (ε=${config.epsilonPerRound}/round, δ=${config.delta}) → cumulative ε=${epsCum.toFixed(1)}`, r);
    }
    ...
    if (config.secureAgg) log("priv", `Masks cancelled in aggregate — individual updates remain unreadable to server`, r);
    log("ok", `${config.algo.toUpperCase()} aggregate committed from ${updates.length} masked updates`, r);

    // Centralized reference baseline (pooled data)
    const cLocal = localTrain(ds.X, ds.y, trainIdx, cw, cb, config.localEpochs, config.learningRate, 0, cw);
    cw = cLocal.w;
    cb = cLocal.b;

    if (isCentral) {
      w = Float64Array.from(cw);
      b = cb;
    }
    ...
    log(
      "ok",
      `Global model (${config.algo.toUpperCase()}) · acc ${(fed.accuracy * 100).toFixed(1)}% · F1 ${fed.f1.toFixed(3)} · AUC ${fed.auc.toFixed(3)} · loss ${fed.loss.toFixed(3)}`,
      r
    );
    await sleep(config.speedMs * 0.35);
  }
  ...
  const result: RunResult = {
    ...
    status: "completed",
    modelName: `${ds.meta.name} (${config.algo.toUpperCase()})`,
    byGuest: false,
  };
  return result;
}
