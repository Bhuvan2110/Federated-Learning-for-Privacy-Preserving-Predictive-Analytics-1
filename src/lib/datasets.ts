...
export function partitionDataset(
  ...
  trainIdx: number[],
  disabled: Set<string>,
  customClients?: Record<string, any[]>,
  ...
  let clients: ClientInfo[] = splits.map((s, k) => {
  ...
  const deletedSet = new Set(deletedClients?.[ds.meta.id] ?? []);
  const customList = customClients?.[ds.meta.id] ?? [];

  // Filter out deleted clients
  if (deletedSet.size > 0) {
    const keptIdx: number[] = [];
    clients = clients.filter((c, idx) => {
      const keep = !deletedSet.has(c.id);
      if (keep) keptIdx.push(idx);
      return keep;
    });
    const keptSplits = keptIdx.map((i) => splits[i]);
    splits.length = 0;
    splits.push(...keptSplits);
  }

  // Append custom clients
  customList.forEach((cc, idx) => {
    if (!deletedSet.has(cc.id)) {
      const positives = Math.round(cc.nSamples * (cc.positiveRate || 0.3));
      const pr = cc.positiveRate || 0.3;
      const epsKl = 1e-9;
      const drift =
        pr * Math.log((pr + epsKl) / (globalPos + epsKl)) +
        (1 - pr) * Math.log((1 - pr + epsKl) / (1 - globalPos + epsKl));
      ...
      clients.push({
        ...
        org: cc.org,
        region: cc.region,
        nSamples: cc.nSamples,
        positives,
        positiveRate: pr,
        enabled: !disabled.has(cc.id),
        drift,
      });

      const customSplit = trainIdx.slice((idx + nClients) * 30, (idx + nClients) * 30 + cc.nSamples);
      splits.push(customSplit.length > 0 ? customSplit : trainIdx.slice(0, 50));
    }
  });
