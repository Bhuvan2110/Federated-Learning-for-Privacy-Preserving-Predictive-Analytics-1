...
export function Toggle({
  ...
  return (
    <label className={cn("flex items-start justify-between gap-3 cursor-pointer select-none", disabled && "opacity-40 cursor-not-allowed")}>
      {label || sub ? (
        <div>
          {label && <div className="text-[13px] font-medium text-fog-200">{label}</div>}
          {sub && <div className="text-[11px] text-fog-500 mt-0.5 leading-snug">{sub}</div>}
        </div>
      ) : null}
      <button
        type="button"
        ...
        disabled={disabled}
        className={cn(
          "relative w-10 h-[22px] rounded-full transition-colors duration-200 shrink-0 mt-0.5",
...
export function SliderRow({
  ...
  return (
    <div className={cn(disabled && "opacity-40 pointer-events-none")}>
      ...
      />
      {noteText && <p className="text-[11px] text-fog-600 mt-1">{noteText}</p>}
    </div>
  );
}
